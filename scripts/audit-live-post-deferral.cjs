...
const REPORTS_DIR = path.resolve(__dirname, '../lighthouse-reports/post-deferral');
...
console.log('========================================================================');
console.log('🚀 RUNNING LIVE POST-DEFERRAL LIGHTHOUSE AUDIT (8 PAGES, MOBILE)');
console.log('========================================================================\n');

async function run() {
  for (const page of PAGES) {
    const targetUrl = `${BASE_URL}${page.path}?v=post_deferral_v1`;
    ...
    try {
      ...
      console.log(`    Long tasks count: ${longTasks.length} (Blocking sum: ${longTasks.reduce((s, t) => s + t.blockingMs, 0)}ms)\n`);
    } catch (err) {
      console.error(`  ❌ Failed: ${page.name}: ${err.message}`);
    }
  }

  fs.writeFileSync(path.join(REPORTS_DIR, 'summary-post-deferral.json'), JSON.stringify(results, null, 2), 'utf8');
  console.log('========================================================================');
  console.log('✅ Post-deferral suite complete! Summary written to lighthouse-reports/post-deferral/summary-post-deferral.json');
  console.log('========================================================================');
}
