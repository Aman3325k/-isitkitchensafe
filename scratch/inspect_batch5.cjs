...
const targetSlugs = ['sweet-potato', 'tomato', 'yellow-squash', 'zucchini', 'mashed-potatoes'];
...
console.log('\n=== SEARCHING FOR "Prepared Food Product" OR PREPARED FOOD IN EN/ES/PT ===');
en.forEach(i => {
  if (i.material && (i.material.toLowerCase().includes('prepared') || i.material.toLowerCase().includes('product') || i.slug.includes('mashed'))) {
    console.log(`EN [${i.appliance}] ${i.slug}: material="${i.material}" item="${i.item}"`);
  }
});

es.forEach(i => {
  if (i.material && (i.material.toLowerCase().includes('prepar') || i.material.toLowerCase().includes('elabor') || i.material.toLowerCase().includes('product') || i.slug.includes('mashed'))) {
    console.log(`ES [${i.appliance}] ${i.slug}: material="${i.material}" item="${i.item}"`);
  }
});

pt.forEach(i => {
  if (i.material && (i.material.toLowerCase().includes('prepar') || i.material.toLowerCase().includes('elabor') || i.material.toLowerCase().includes('produt') || i.slug.includes('mashed'))) {
    console.log(`PT [${i.appliance}] ${i.slug}: material="${i.material}" item="${i.item}"`);
  }
});
