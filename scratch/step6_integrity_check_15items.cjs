...
console.log('=== STEP 6: FULL-FILE INTEGRITY CHECK (ALL 15 CATEGORY 2 ITEMS COMBINED) ===\n');
...
// 2. Confirm item count is exactly baseline (671) + 5 = 676
const oldBaseline = 671;
const expectedCount = 676;
...
console.log('   ✓ Both files match exact expected count (676).\n');

// 3, 4, 5. Re-run strict validator across all 15 items
const all15Slugs = [
...
console.log('3-5. Strict validator pass across all 15 items (reading fresh from disk):');

all15Slugs.forEach((slug, idx) => {
...
// 6. Zero duplicate slugs across the entire freezer subset
console.log('\n6. Full duplicate slug scan across ENTIRE freezer subset in both files:');
...
if (duplicatesEs.length > 0) {
  ...
  console.log('   ✓ Zero duplicate freezer slugs in items.es.json (unique slugs:', seenEs.size, ')');
}
...
if (duplicatesPt.length > 0) {
  ...
  console.log('   ✓ Zero duplicate freezer slugs in items.pt.json (unique slugs:', seenPt.size, ')');
}

console.log('\n' + '='.repeat(60));
console.log(`SUMMARY: ${totalChecks} total checks executed across all 15 items.`);
