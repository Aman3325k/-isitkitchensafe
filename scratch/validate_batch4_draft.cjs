...
const draft = JSON.parse(fs.readFileSync('scratch/cat3_batch4_translations.json', 'utf8'));
...
['es', 'pt'].forEach(lang => {
  ...
  items.forEach((item, idx) => {
    ...
    if (slug === 'potato') {
      expectedMaterial = lang === 'es' ? 'Tubérculo con Almidón' : 'Tubérculo com Amido';
    } else {
      expectedMaterial = lang === 'es' ? 'Verdura Fresca' : 'Vegetal Fresco';
    }
    ...
    // Check depends nuance for potato
    if (slug === 'potato') {
