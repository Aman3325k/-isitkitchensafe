...
const draftPath = 'scratch/cat3_batch5_translations.json';
...
const expectedSlugs = ['sweet-potato', 'tomato', 'yellow-squash', 'zucchini', 'mashed-potatoes'];
...
['es', 'pt'].forEach(lang => {
  ...
  items.forEach((item, idx) => {
    ...
    // If tomato, check depends nuance
    if (slug === 'tomato') {
      assert(item.safe === 'depends', `[${lang}][tomato] safe is depends`);
      assert(item.shortAnswer.toLowerCase().includes(lang === 'es' ? 'depende' : 'depende'), `[${lang}][tomato] shortAnswer starts with Depende`);
      const text = (item.reason + ' ' + item.shortAnswer).toLowerCase();
      assert(text.includes(lang === 'es' ? 'salsas' : 'molhos') || text.includes(lang === 'es' ? 'cocin' : 'cozid'), `[${lang}][tomato] mentions cooked/sauces nuance`);
    }
  });
});
