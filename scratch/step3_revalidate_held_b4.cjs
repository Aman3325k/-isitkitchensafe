...
const b4Path = 'scratch/cat2_batch4_translations.json';
const b4Data = JSON.parse(fs.readFileSync(b4Path, 'utf8'));
...
console.log('=== STEP 3: INDEPENDENT RE-VALIDATION OF HELD DRAFT (scratch/cat2_batch4_translations.json) ===\n');

const slugs = ['pineapple', 'plum', 'raspberry', 'strawberry', 'watermelon'];

slugs.forEach(slug => {
  const enItem = freezerItems.find(i => i.slug === slug);
  const trans = b4Data[slug];
  if (!enItem) {
...
if (failures === 0) {
  console.log('STATUS: STEP 3 VALIDATION PASSED WITH 0 FAILURES (100% CLEAN)');
} else {
  console.error('STATUS: STEP 3 VALIDATION FAILED');
  process.exit(1);
}
